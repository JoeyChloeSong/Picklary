...
module.exports = function applyMlpPlayoffsV067(board) {
  ...
  const projectedPaths = [
    ...
    { seed:4, team:'Columbus Sliders', opponent:'Brooklyn Pickleball Team', note:'The remaining teams form a depth-heavy quarterfinal in the seed-order scenario.', noteKo:'남은 두 팀은 선수층이 강한 8강 대진을 구성하게 됩니다.' }
  ];
  ...
  upsert(board.tournaments, 'mlp-playoffs-dallas-2026', Object.assign({
    ...
    resultNoteKo:'아직 시리즈가 시작되지 않았습니다. 대진, 일정 시간대, 로스터와 Picklary 대진 분석은 첫 경기 전 최신 공개 정보를 반영합니다.',
    ...
    watchKo:['1차전과 2차전 사이의 라인업 조정','홈팀의 혼합복식·DreamBreaker 라인업 후공 선택권','하위 시드가 단식 선수층을 앞세워 결정전까지 끌고 갈 수 있는지','승자 4팀의 Newport Beach 재시드 방식'],
    ...
    storylineKo:'Dallas는 고정 토너먼트 대진표가 아닙니다. 시리즈 승자를 재시드한 뒤 상위 4팀이 8강 상대를 선택하므로 한 번의 업셋이 다음 라운드 전체 지도를 바꿉니다.',
    ...
    stories:[
      { kicker:'SERIES FORMAT', kickerKo:'시리즈 경기 방식', title:'The second match is a coaching test, not a replay', titleKo:'2차전은 재경기가 아니라 코칭 테스트', body:'Lineup order, targeting, and DreamBreaker choices can all change after the opening match reveals the pressure point.', bodyKo:'1차전에서 약점이 드러난 뒤 라인업 순서, 타깃과 DreamBreaker 선택이 모두 바뀔 수 있습니다.' },
    ...
    sourceUrl:officialDallas, sourceName:'Official MLP Dallas Playoffs', sourceNameKo:'MLP Dallas 플레이오프 공식 페이지', secondaryUrl:officialStandings, secondaryName:'Final regular-season standings', secondaryNameKo:'MLP 정규시즌 최종 순위'
  }, commonPlayoffFacts));
  ...
  upsert(board.tournaments, 'mlp-finals-new-york-city-2026', {
    ...
    watchKo:['준결승 상대 선택','3일간 파이널 주말의 라인업 피로','우승 압박 속 DreamBreaker 선수층'],
    ...
    sourceUrl:officialFinals, sourceName:'Official MLP Finals NYC', sourceNameKo:'MLP New York City 파이널 공식 페이지', secondaryUrl:officialNewport, secondaryName:'Newport Beach quarterfinals', secondaryNameKo:'Newport Beach 8강 공식 페이지'
  });
  ...
  if (orlando) Object.assign(orlando, {
    ...
    resultChecked:'2026-08-05 · final standings verified', resultCheckedKo:'2026-08-05 · 최종 순위 확인',
    ...
    resultNoteKo:'Orlando 대회를 끝으로 정규시즌이 종료됐습니다. 아래 표에서 플레이오프 진출 12팀과 각 팀의 포스트시즌 시작 라운드를 확인할 수 있습니다.',
    ...
    overviewKo:'New Jersey가 118점으로 정규시즌 1번 시드를 차지했습니다. St. Louis, Los Angeles, Columbus도 Dallas 1라운드를 건너뛰며, 5–12번 시드는 1라운드부터 시작합니다.',
    results:[], standings, rosters,
    sourceNameKo:'MLP Orlando 공식 대회 페이지', secondaryNameKo:'MLP 공식 순위',
    ...
    notableFactsKo:['New Jersey가 118 정규시즌 순위 포인트로 1위를 확정했습니다.','St. Louis, Los Angeles, Columbus가 상위 4팀을 완성했습니다.','Brooklyn, Dallas, Palm Beach, Texas가 5–8번 시드를 받았습니다.','Atlanta, SoCal, Las Vegas, Chicago가 9–12번 시드를 채웠습니다.','정규 시즌은 종료됐고 다음 공식 일정은 Dallas 1라운드입니다.'],
    ...
    watchKo:['상위 4팀의 회복 주간 활용','Dallas에서 첫 업셋을 만드는 하위 시드','단일 매치에서 3전 2선승제로 바뀔 때 선수층의 영향'],
    ...
    storylineKo:'Orlando가 진출 팀을 확정했지만 전통적인 고정 대진표는 만들지 않았습니다. 상대 선택권 때문에 매 라운드 후 포스트시즌 경로가 다시 열립니다.',
