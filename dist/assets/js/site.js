...
(function () {
  ...
  function drawScene(s) {
    ...
    var youArr = s.you || [], meIdx = -1;
    if (youArr.length) {
      if (ball && ball.to) {
        var bx = ball.to.x, by = ball.to.y, bestD = Infinity;
        youArr.forEach(function (p, i) { var d = (p.x - bx) * (p.x - bx) + (p.y - by) * (p.y - by); if (d < bestD) { bestD = d; meIdx = i; } });
      } else { meIdx = 0; }
    }
    youArr.forEach(function (p, i) { markers.appendChild(dot(p.x, p.y, "court-dot " + (i === meIdx ? "court-dot--me" : "court-dot--you"))); });
